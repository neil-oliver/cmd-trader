...
module.exports = {
  ...
  async mainMenu(){
    ...
    if (answers.main == "Output Rankings"){
      ...
      arbitrage.outputResults(this.accounts)
      this.mainMenu()
  ...
  async intervalSelect(){
    ...
    if (answers.interval.length > 0){
      ...
      const res = await inquirer.cancelContinue('You did not select any intervals.');
      if (res.cancel == "Yes"){
        this.mainMenu()
      } else {
        this.intervalSelect()
  ...
  async install(){
    ...
    for (let exchange of config.exchanges){
      ...
      for (let pair of info.symbols){
        ...
        if (quoteCheck && !config.exclusionList.includes(pair.baseAsset)) this.accounts[exchange].pair = {base: pair.baseAsset, quote: pair.quoteAsset}
      }
