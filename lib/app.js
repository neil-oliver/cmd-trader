  ...
  async install(){
    ...
    for (let exchange of config.exchanges){
      ...
      for (let pair of info.symbols){
        if (pair.quoteAsset == config.preferredQuote && !config.exclusionList.includes(pair.baseAsset)) this.accounts[exchange].pair = {base: pair.baseAsset, quote: pair.quoteAsset}
      }
